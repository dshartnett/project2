...
var object_arr = [];
for (var i = 0; i < 1; i++) object_arr[i] = new g_object(CANVAS_WIDTH/2 + 600*Math.random()-300,CANVAS_HEIGHT/2 + 600*Math.random()-300, 50, i%2?-500:500);
for (var i = 1; i < OBJECT_NUM-2; i++) object_arr[i] = new l_object(CANVAS_WIDTH/2 + 600*Math.random()-300,CANVAS_HEIGHT/2 + 600*Math.random()-300, 30, Math.random()*2*Math.PI);
...
for (var i = 0; i < PARTICLE_NUM; i++)
 parArr[i] = new particle(Math.random()*CANVAS_WIDTH, Math.random()*CANVAS_HEIGHT, 100, i%3==0?100:(i%3==1?300:500),i%3==0?"black":(i%3==1?"purple":"lime")/*i%2?100:300,i%2?"black":"lime"*/,4);
...
function changeParCount() {
	...
	parArr = [];
	for (var i = 0; i < PARTICLE_NUM; i++) parArr[i] = new particle(Math.random()*CANVAS_WIDTH, Math.random()*CANVAS_HEIGHT, 100, i%3==0?100:(i%3==1?300:500),i%3==0?"black":(i%3==1?"purple":"lime")/*i%2?100:300,i%2?"black":"lime"*/,4);
}
        ...
        function c_object(xpos, ypos, side)
        {
			...
			this.update_object = function(parArr)
			{
				...
				for (var i = 0; i < PARTICLE_NUM; i++)
				{
					//disX = parArr[i].X - this.X;
					//disY = parArr[i].Y - this.Y;
					
					if (parArr[i].X > this.X && parArr[i].X < this.X+this.side && parArr[i].Y > this.Y && parArr[i].Y < this.Y+this.side)
					{
						switch (parArr[i].pcolor)
						{
							case "black": parCountB++; break;
							case "purple": parCountP++; break;
							case "lime": parCountL++; break;
						}
						//parArr[i].pcolor=="black"?parCountB++:(parArr[i].pcolor=="purple"?parCountP++:parCountL++);
					}					
				}
        ...
        function l_object(xpos, ypos, rad, angle)
        {
			...
			this.update_object = function(parArr)
			{
				...
				for (var i = 0; i < PARTICLE_NUM; i++)
				{
					disX = this.X - parArr[i].X;
					disY = this.Y - parArr[i].Y;
					if (Math.abs(disX) < this.radius && Math.abs(disY) < this.radius)
					{
						distance2 = disX*disX + disY*disY;
						if (distance2 < this.radius*this.radius)
						{
							parArr[i].X = this.X+this.radius*Math.cos(l_angle) + 5*Math.random() - 2.5;
							parArr[i].Y = this.Y+this.radius*Math.sin(l_angle) + 5*Math.random() - 2.5;
							parArr[i].vX = shoot_speed*Math.cos(l_angle) + .4*Math.random() - .2;
							parArr[i].vY = shoot_speed*Math.sin(l_angle) + .4*Math.random() - .2;
						}
					}
				}
			}
			...
			this.update_object = function(parArr)
			{
				...
				for (var i = 0; i < PARTICLE_NUM; i++)
				{
					...
					if (distance2 > this.radius*this.radius)
					{
						...
						force = this.charge*parArr[i].charge/distance2;
						parArr[i].vX += disX*force/distance/parArr[i].mass;
						parArr[i].vY += disY*force/distance/parArr[i].mass;
					}
					else
					{
						//	parArr[i].vX += 0.00001*disX*parArr[i].mass;
						//	parArr[i].vY += 0.00001*disY*parArr[i].mass;
						parArr[i].vX += C_ROTATION*(disX-disY);//-0.001*disY + 0.001*disX;
						parArr[i].vY += C_ROTATION*(disX+disY);//0.001*disX + 0.001*disY;
						parArr[i].vX *= C_FRICTION;
						parArr[i].vY *= C_FRICTION;
					}
				}
			}
...
function update()
{
	...
	*/
	for (var i = 0; i < PARTICLE_NUM; i++) parArr[i].updatep();
	for (var i = 0; i < OBJECT_NUM; i++) object_arr[i].update_object(parArr);
}
...
function draw()
{
	...
	for (var i = 0; i < PARTICLE_NUM; i++) parArr[i].drawp();
